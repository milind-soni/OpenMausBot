...
function Shell() {
  ...
  // settings, and app settings overlays cover the same idea from a different trigger: close the
  // drawer whenever an action opens something over the chat.
  useEffect(() => {
    setDrawerOpen(false);
  }, [state.selectedId, bot?.threadId, group?.threadId, state.activeView, overlayOpen(state, "plugins"), overlayOpen(state, "settings"), overlayOpen(state, "appSettings")]);
