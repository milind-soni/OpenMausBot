...
/** A visual block can prepare a follow-up without sending a turn, replacing
 * the user's draft, dropping attachments, or changing the draft's mode. */
export function appendComposerDraftText(id: string, text: string): void {
  const store = getStore();
