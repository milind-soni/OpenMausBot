  ...
  return (
    ...
    <div className="group relative" title={iconOnly ? bot.name : undefined}>
      ...
      {!renaming && !deleting && !iconOnly && <>
        ...
        <button type="button" aria-label={t("sidebar.bot.actions", { name: bot.name })} title={t("sidebar.bot.actions", { name: bot.name })} aria-haspopup="menu" aria-expanded={menuOpen} onClick={(event) => { const rect = event.currentTarget.getBoundingClientRect(); onMenu({ botId: bot.id, x: rect.left, y: rect.bottom }); }}
