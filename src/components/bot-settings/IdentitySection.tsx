...
export function IdentitySection({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Field label={t("botSettings.name")}>
        <input
          className={inputCls}
      ...
      <Field label={t("botSettings.jobTitle")}>
        <input
          className={inputCls}
          maxLength={BOT_PROFILE_LIMITS.title}
          placeholder={t("botSettings.titlePlaceholder")}
          value={bot.title}
      ...
      <div className="block">
        <div className="mb-1.5 flex items-center justify-between gap-3">
          ...
          >
            <BookOpen size={12} /> {t("botSettings.viewFull")}
          </button>
        </div>
        <textarea
          ...
          placeholder={t("botSettings.blurbPlaceholder")}
          aria-label={t("botSettings.blurb")}
          value={bot.description}
          onChange={(e) => patch({ description: e.target.value })}
        />
        <div className="mt-1.5 flex items-start justify-between gap-3 text-[11px] text-ink-secondary">
          <span>{t("botSettings.blurbHint")}</span>
