      ...
      {renaming ? (
        <div className="flex items-center gap-1 px-2 py-1">
          ...
          <button
            type="button"
            onClick={saveRename}
            aria-label={t("Save channel name")}
            title={t("Save")}
          ...
          <button
            type="button"
            onClick={onClose}
            aria-label={t("Cancel channel rename")}
            title={t("Cancel")}
            className="flex size-8 shrink-0 items-center justify-center rounded-lg text-ink-secondary hover:bg-raised hover:text-ink"
          >
            <X size={15} />
  ...
  return (
    ...
    >
      <div className="w-[340px] rounded-2xl border border-hairline/50 bg-card p-4 shadow-2xl">
        <div className="mb-3 text-[15px] font-semibold text-ink">{t("New Channel")}</div>
        <input
          autoFocus
          maxLength={100}
          value={name}
          ...
          placeholder={t("Channel name (for example, Website launch)")}
        ...
        <input
          ...
          placeholder={t("Context (optional): Work, Personal, Client…")}
          aria-label={t("Channel context")}
          className="mb-3 w-full rounded-lg bg-raised/70 px-3 py-2 text-[14px] text-ink placeholder:text-ink-secondary focus:outline-none"
        />
        <BotPickerList
          bots={bots}
          picked={picked}
          onToggle={toggle}
          emptyHint={t("Create a bot first — channels are made of bots.")}
        />
        <button
          onClick={create}
        ...
        >
          {t("Create Channel")}{picked.size ? ` · ${picked.size} ${picked.size === 1 ? t("bot") : t("bots")}` : ""}
        </button>
      </div>
    </div>
  );
    ...
    >
      ...
      >
        <input
          ...
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder={t("New context…")}
          aria-label={t("New context name")}
