...
if [ -n "${ASTRA_POSTINSTALL_TEST_ROOT:-}" ]; then
  ...
  case "$TEST_ROOT" in
    /tmp/*) ;;
    *) echo "Astra test install root must stay under /tmp" >&2; exit 1 ;;
  esac
...
if [ "$TEST_MODE" -eq 0 ]; then
  if command -v update-alternatives >/dev/null 2>&1; then
    update-alternatives --remove astra /opt/Astra/astra
  elif [ -L /usr/bin/astra ] && [ "$(readlink /usr/bin/astra)" = /opt/Astra/astra ]; then
    rm -- /usr/bin/astra
  fi
fi

profile=$APPARMOR_DIR/astra-browser
if [ ! -e "$profile" ] && [ ! -L "$profile" ]; then exit 0; fi
if [ -L "$APPARMOR_DIR" ] || [ -L "$profile" ] || [ ! -f "$profile" ]; then
  echo "Astra AppArmor profile is unsafe; refusing to remove it: $profile" >&2
  exit 1
fi
if [ "$TEST_MODE" -eq 0 ] && [ -x /usr/bin/ischroot ] && /usr/bin/ischroot; then
...
elif [ -r "$APPARMOR_PROFILES" ] && ! grep -q '^astra-browser ' "$APPARMOR_PROFILES"; then
  ...
  if ! "$APPARMOR_PARSER" -R "$profile"; then
    echo "Astra could not unload its browser AppArmor profile: $profile" >&2
    exit 1
  fi
elif [ -r "$APPARMOR_PROFILES" ]; then
  echo "Astra needs apparmor_parser to unload its browser profile: $profile" >&2
  exit 1
fi
rm -- "$profile"
