...
data class PairingInvite(val connection: Connection, val credential: String) {
    companion object {
        ...
        fun parse(url: URI): PairingInvite? {
            if (!url.scheme.equals("astra", ignoreCase = true) ||
        ...
        private fun credential(values: Map<String, String>): String? {
            values["token"]?.let { token ->
                ...
                val prefix = if (token.startsWith("astra_pair_")) "astra_pair_" else "omb_pair_"
                val suffix = token.removePrefix(prefix)
                if (!token.startsWith(prefix) || token.toByteArray().size != 52 || suffix.length != 43) {
                    return null
                }
