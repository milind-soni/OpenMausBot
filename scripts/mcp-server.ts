#!/usr/bin/env node
// Model Context Protocol (MCP) Server for Astra
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid Astra URL: '${url}'`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("Astra URL must use http:// or https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("Astra URL must not contain credentials; use ASTRA_TOKEN instead");
  }
  if ((parsed.pathname !== "/" && parsed.pathname !== "") || parsed.search || parsed.hash) {
    throw new Error("Astra URL must be an origin without a path, query, or fragment");
  }
...
const configuredUrl = process.env.ASTRA_URL ||
  (process.env.ASTRA_PORT ? `http://127.0.0.1:${process.env.ASTRA_PORT}` : undefined);

export const ASTRA_BASE_URL = validateBaseUrl(configuredUrl || "http://127.0.0.1:8799");
...
export function log(msg: string) {
  process.stderr.write(`[astra-mcp] ${msg}\n`);
}

export const DEFAULT_REQUEST_TIMEOUT_MS = 30_000;

function requestTimeoutMs(): number {
  const raw = Number(process.env.ASTRA_MCP_TIMEOUT_MS);
...
function requestHeaders(options: RequestInit): NonNullable<RequestInit["headers"]> {
  const token = process.env.ASTRA_TOKEN?.trim();
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 && !process.env.ASTRA_TOKEN?.trim()) {
      throw new Error(
        "Astra refused this write because the installed desktop app requires a paired session token. " +
        "Set ASTRA_TOKEN as described in docs/mcp-server.md.",
      );
    }
    throw new Error(`Astra API error (${response.status}): ${text || response.statusText}`);
  }
  try {
    return await response.json();
  } catch {
    throw new Error(`Astra API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  for (const unvalidated of candidates) {
    ...
    try {
      ...
      if (health?.app !== "astra") {
        failures.push(`${candidate} answered, but it was not Astra`);
        continue;
      }
      return candidate;
    } catch (error) {
  ...
  throw new Error(`Could not find a running Astra server. ${failures.join("; ")}`);
}

export async function resolveBaseUrl(): Promise<string> {
  if (discoveredBaseUrl) return discoveredBaseUrl;
  if (process.env.ASTRA_TOKEN?.trim() && !configuredUrl) {
    throw new Error("Set ASTRA_URL or ASTRA_PORT when using ASTRA_TOKEN so credentials are never sent during port discovery");
  }
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the Astra server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid Astra ID`);
  return value;
}
...
export async function handleToolCall(
  ...
  switch (name) {
    case "get_system_health": {
      ...
      if (res?.app !== "astra") throw new Error("The configured endpoint is not an Astra server");
      return {
        status: "connected",
        endpoint: discoveredBaseUrl ?? ASTRA_BASE_URL,
        app: "astra",
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("Astra did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("Astra did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("Astra did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("Astra did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("Astra did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("Astra did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
    ...
    case "set_bot_model": {
      ...
      if (args.task_id !== undefined) {
        ...
        if (!isRecord(res?.task)) throw new Error("Astra did not return the updated task");
  ...
  try {
    ...
    if (method === "initialize") {
      ...
      return formatResponse(id, {
        ...
        instructions: "Use bounded read tools before mutating the Astra team. Approval grants, deletion, and computer lifecycle are intentionally unavailable.",
      });
    }
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("Astra MCP server running on stdio");
}
