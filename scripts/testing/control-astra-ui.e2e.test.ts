...
// machine. It spawns the real `control-astra ui launch` (a child it can Ctrl-C),
...
// directory, ASTRA_AGENT_BROWSER_PATH or PATH) or when ASTRA_UI_E2E=1 asks for the
...
const CLI = join(ROOT, "scripts", "control-astra.ts");
...
if (!enabled) {
  console.log(`skipping control-astra ui e2e: no agent-browser binary resolves (looked in ${UI_TOOLS_DIR}, ASTRA_AGENT_BROWSER_PATH and PATH); set ASTRA_UI_E2E=1 to install the pinned release`);
}
...
// ASTRA_UI_EVIDENCE_DIR keeps the screenshot (CI uploads it); otherwise it is temporary.
const evidenceDir = process.env.ASTRA_UI_EVIDENCE_DIR ? resolve(ROOT, process.env.ASTRA_UI_EVIDENCE_DIR) : mkdtempSync(join(tmpdir(), "omb-ui-evidence-"));
const ownsEvidenceDir = !process.env.ASTRA_UI_EVIDENCE_DIR;
...
describe("control-astra ui drives the real renderer", () => {
  ...
  run("sends a turn from the composer and shows the reply and the scripted tool chip", async () => {
    ...
    expect(title).toMatchObject({ ok: true, result: "Isolated Astra Chat" });
