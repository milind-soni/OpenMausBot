// `control-astra ui`: drive the real React renderer headlessly against the
...
//
// Imported by scripts/control-astra.ts, which owns HELP and the MUTATING set;
...
const ENTRIES = {
  threads: { entry: "/scripts/testing/threads-preview.tsx", route: "/__threads.html", title: "Isolated Astra Chat" },
...
/** ASTRA_AGENT_BROWSER_PATH, then the tools directory, then PATH; otherwise the
...
export async function runControlOmbUi(args: string[]): Promise<unknown> {
  ...
  if (verb === "launch") {
    throw new ControlOmbError("ui launch is available only from the executable CLI", "run `node --experimental-strip-types scripts/control-astra.ts ui launch`");
  }
  ...
  throw new ControlOmbError(`unknown ui command ${JSON.stringify(verb)}`, "run control-astra ui help");
}
