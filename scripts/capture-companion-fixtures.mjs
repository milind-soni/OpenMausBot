...
// reads or writes your real ~/.astra. No real name, key or token can end up in a fixture. The
...
async function main() {
  ...
  const harness = start("harness", [join(ROOT, "server", "index.ts")], {
    ...
    ASTRA_WEBHOOK_PORT: String(base + 1),
  });
  ...
  const sidecar = start("companion", [join(ROOT, "companion", "src", "index.ts")], {
    ...
    ASTRA_COMPANION_DIR: join(home, "companion"),
  });
