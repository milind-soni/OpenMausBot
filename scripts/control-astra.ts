...
import { launchUi, runControlOmbUi } from "./testing/control-astra-ui.ts";
...
export const HELP_UI = `renderer (needs a ui launch handle; every verb takes --ui HANDLE, never discovery):
  node --experimental-strip-types scripts/control-astra.ts ui launch [--entry threads] [--tool-calls JSON] [--mode happy]
...
export const HELP = `control-astra — verify a running Astra instance through its shared MCP core
...
mutating (an explicit --url or ASTRA_URL/ASTRA_PORT is required):
...
export function parse(
  ...
  try {
    ...
    throw new ControlOmbError(
      ...
      `run control-astra help for the ${command} syntax`,
    );
  }
}
...
function configuredUrl(raw: unknown, env: NodeJS.ProcessEnv, requiredForMutation: boolean): string | undefined {
  const explicit = typeof raw === "string" && raw.trim()
    ? raw.trim()
    : env.ASTRA_URL?.trim() || (env.ASTRA_PORT ? `http://127.0.0.1:${env.ASTRA_PORT}` : "");
  if (!explicit) {
    if (requiredForMutation) {
      throw new ControlOmbError(
        "mutating commands require an explicit Astra instance",
        "start `control-astra launch`, then pass its URL with --url",
      );
    }
    return undefined;
  }
...
export async function runControlOmb(
  ...
  throw new ControlOmbError(`unknown command ${JSON.stringify(command)}`, "run control-astra help");
}
...
export async function launchVerificationServer(
  ...
  const dataDir = mkdtempSync(join(browser && process.platform !== "win32" ? "/tmp" : tmpdir(), "astra-verify-data-"));
  ...
  const evidenceDir = join(tmpdir(), "astra-verification-evidence");
  ...
  if (localVm) Object.assign(childEnv, {
    ASTRA_EXTRA_PATH: [localVm.binDir, ...(process.platform === "win32" ? [join(childEnv.SYSTEMROOT || "C:\\Windows", "System32")] : [])].join(delimiter),
    CONTAINER_HOST: localVm.host,
    CONTAINER_SSHKEY: localVm.sshKey,
    ASTRA_STATIC_DIR: localVm.staticDir,
  });
  if (enterprise) Object.assign(childEnv, { ASTRA_ENTERPRISE_DIR: enterprise.dir, ASTRA_LICENSE_KEY: enterprise.licenseKey });
  if (browser) Object.assign(childEnv, {
    ASTRA_AGENT_BROWSER_PATH: browser.binaryPath,
...
function requireForegroundTerminal(command: string): void {
  if (process.env.npm_lifecycle_event === "control:omb") {
    throw new ControlOmbError(
      ...
      `run \`node --experimental-strip-types scripts/control-astra.ts ${command}\``,
    );
  }
}
