  ...
  try {
    ...
    if (!live) {
      ...
      for (const [index, group] of organization.entries()) {
        ...
        const assign = (bot: any) => ({ tool: "coordinate_bots", arguments: { bot_ids: [bot.id], message: `${group.name}決定に基づく担当業務。実行してください`, request_key: `owner-${bot.id}` } });
        ...
        for (const [memberIndex, member] of [one, two].entries()) {
          ...
          if (to !== undefined) {
            turns.push({ expectContextIncludes: [`${group.name}決定`], steps: [{ tool: "coordinate_bots", arguments: { group_id: rooms[to].id, bot_ids: [teams[to][0].id], message: `${group.name}決定を具体化した依頼。${organization[to].topic}`, request_key: `to-${to}` } }], reply: `${member.name}担当として下層へ依頼しました。` });
