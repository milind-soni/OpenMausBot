 ...
 * record here:
 *
 * - **Shared records (§10.2).** The PTR of `_astra._tcp.local` is
...
export interface ServiceInfo {
  ...
  type: string;
  port: number;
  /** the name our A records claim, e.g. "astra-1a2b3c4d.local" */
  host: string;
  addresses: string[];
...
export function dnsLabel(text: string, fallback = "Astra"): string {
  let label = text
