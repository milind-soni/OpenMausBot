...
const forwardHeaders = (req: IncomingMessage, authenticatedDeviceId?: string, mutationToken?: string): Record<string, string> => {
  ...
  if (authenticatedDeviceId && /^[\w-]{1,128}$/.test(authenticatedDeviceId)) {
    out["x-astra-companion-device"] = authenticatedDeviceId;
    if (mutationToken) out["x-astra-companion-auth"] = mutationToken;
  }
  ...
  const handle = function handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (isCloudDesktopAccess(method, path) && !device?.cloudDesktopAccess) {
      return sendJson(res, 403, {
        error: "cloud desktop access is off for this device — enable it in Astra → Settings → Remote access",
      });
    }
      ...
      (harness) => {
        ...
        if (method === "GET" && path === "/api/health") {
          ...
          const fail = () => {
            if (finished) return;
            finished = true;
            sendJson(res, 502, { error: "Astra is not ready on this computer" });
          };
          ...
          harness.on("end", () => {
            ...
            if (
              ...
              (identity as { app?: unknown } | null)?.app !== "astra"
            ) {
              fail();
              return;
            }
            finished = true;
            sendJson(res, 200, { app: "astra" });
          });
          return;
        }
        ...
        harness.on("data", (chunk: Buffer) => {
          ...
          if (size > MAX_JSON_BODY_BYTES) {
            ...
            else sendJson(res, 502, { error: "the response from Astra was too large" });
            return;
          }
          chunks.push(chunk);
        });
