...
// on the companion's stdin rather than the line-oriented JSON wire in wire.ts.
// One frame is a 4-byte big-endian header length, a JSON header, then exactly
// the payload bytes that header declares — no delimiter a payload could forge,
// and no base64 inflation against the size ceilings.
//
// server/worker-task-frames.ts is the other end of this format. They are
// duplicated rather than imported for the reason platform.ts gives, and
// test/frame-parity.test.ts drives one against the other so they cannot drift.
...
export function encodeFrameHeader(header: FrameHeader): Buffer {
  ...
  const canonical = {
    kind: header.kind,
    bytes: header.bytes,
    ...(header.path === undefined ? {} : { path: header.path }),
    ...(header.sha256 === undefined ? {} : { sha256: header.sha256 }),
  };
  const json = Buffer.from(JSON.stringify(canonical), "utf8");
