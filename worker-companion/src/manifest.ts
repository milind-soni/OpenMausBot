...
export const TASK_MAX_LIFETIME_MS = 2 * 60 * 60_000;
...
const manifestSchema = z.object({
  ...
  target: z.object({
    ...
    browserProfile: z.string().min(1).max(100).refine((value) => !/[\u0000-\u001f]/.test(value)),
...
/** Exact browser origin only: no paths, queries, wildcards, credentials, or
 * non-HTTP schemes. Kept byte-for-byte aligned with the control-plane rule. */
...
export function parseStagedManifest(document: JsonValue, expectedSha256: string): TaskManifest {
  ...
  if (manifest.expiresAt <= manifest.createdAt || manifest.expiresAt - manifest.createdAt > TASK_MAX_LIFETIME_MS) {
    throw new Error("staged task manifest has an invalid lifetime");
  }

  const unique = (values: string[], label: string) => {
    if (new Set(values.map((entry) => entry.toLowerCase())).size !== values.length) {
      throw new Error(`staged task ${label} must be unique`);
    }
  };
  ...
  if (!manifest.resultPaths.includes("result.json") || !manifest.resultPaths.includes("changes.patch")) {
    throw new Error("staged task results must include result.json and changes.patch");
  }

  manifest.origins = manifest.origins.map((origin) => {
    const normalized = exactOrigin(origin);
    if (!normalized || normalized !== origin) throw new Error(`staged task origin must be exact: ${origin}`);
    return normalized;
  });
...
const healthTools = (platform: TaskPlatform): readonly string[] =>
...
export function taskCapabilityManifest(manifest: TaskManifest, root: string, issuedAt: number): string {
  ...
  if (manifest.surface === "browser") {
    return [
      ...head,
      ...healthTools(manifest.platform).map((tool) => `    - ${tool}`),
