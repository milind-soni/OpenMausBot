...
/** Immutable copy of the approved inputs. It deliberately sits outside the
 * task root granted to CUA, so an IDE may edit the working copy without also
 * rewriting the evidence checked before later commands and result reads. */
export function taskBaselineRoot(taskId: string, platform: WorkerPlatform = workerPlatform()): string {
  if (!TASK_ID.test(taskId)) throw new Error("invalid task id");
  return join(supportDirectory(platform), "task-baselines", taskId);
}
