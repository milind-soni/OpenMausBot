...
import { HOST_TASK_PLATFORM, parsedManifest, workerFixture } from "../../server/testing/worker-task.ts";
...
function manifestDocument(overrides: Record<string, JsonValue> = {}): JsonValue {
  const manifest = parsedManifest(HOST_TASK_PLATFORM, {
    ...
    ...overrides,
  }, workerFixture(HOST_TASK_PLATFORM), NOW);
...
describe("stage", () => {
  it("writes the manifest and every declared file", async () => {
    ...
    const baseline = taskBaselineRoot(TASK_ID, PLATFORM);
    expect(readFileSync(join(root, "src", "main.txt"), "utf8")).toBe("hello worker");
    expect(readFileSync(join(baseline, "src", "main.txt"), "utf8")).toBe("hello worker");
    expect(JSON.parse(readFileSync(join(root, MANIFEST_FILE), "utf8")).taskId).toBe(TASK_ID);
    expect(JSON.parse(readFileSync(join(baseline, MANIFEST_FILE), "utf8")).taskId).toBe(TASK_ID);
  });
  ...
  it("leaves nothing behind when a stage fails part way", async () => {
    ...
    expect(() => readFileSync(join(taskBaselineRoot(TASK_ID, PLATFORM), MANIFEST_FILE))).toThrow();
  });
  ...
  it("replaces an earlier stage rather than merging into it", async () => {
    ...
    expect(() => readFileSync(join(taskBaselineRoot(TASK_ID, PLATFORM), "src", "main.txt"))).toThrow();
  });
});
...
describe("validate", () => {
  ...
  it("allows the working copy to change while preserving the approved baseline", async () => {
    const document = manifestDocument();
    await stage(stagingStream(document, [goodFile]));
    ...
    expect(readFileSync(join(taskBaselineRoot(TASK_ID, PLATFORM), "src", "main.txt"), "utf8"))
      .toBe("hello worker");
  });

  it("allows the working copy to delete a declared input", async () => {
    const document = manifestDocument();
    await stage(stagingStream(document, [goodFile]));
  ...
  it("refuses an approved baseline changed after staging", async () => {
    const document = manifestDocument();
    await stage(stagingStream(document, [goodFile]));
    writeFileSync(join(taskBaselineRoot(TASK_ID, PLATFORM), "src", "main.txt"), "tampered baseline");
    expect(() => validateTask(TASK_ID, taskManifestDigest(document), PLATFORM))
      .toThrow(/baseline file size changed|baseline file hash changed/);
  });
  ...
  it.skipIf(process.platform === "win32")("refuses a symbolic link added to the working tree", async () => {
    const document = manifestDocument();
    await stage(stagingStream(document, [goodFile]));
    symlinkSync(join(home, "outside"), join(taskRoot(TASK_ID, PLATFORM), "src", "outside-link"));
    expect(() => validateTask(TASK_ID, taskManifestDigest(document), PLATFORM))
      .toThrow(/working tree contains a symbolic link/);
  });
...
describe("run", () => {
  ...
  it("re-validates the baseline and safely runs after an IDE edits the working copy", async () => {
    const document = manifestDocument();
    await stage(stagingStream(document, [goodFile]));
    ...
    await expect(runTaskCommand(TASK_ID, taskManifestDigest(document), "build", PLATFORM))
      .resolves.toMatchObject({ commandId: "build", code: 0 });
  });
});
...
describe("fetch results", () => {
  it("returns only the artefacts the approved manifest declares", async () => {
    ...
    writeFileSync(join(root, "src", "main.txt"), "edited by VS Code");
