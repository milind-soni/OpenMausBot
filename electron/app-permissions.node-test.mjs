...
test("both external-link entry points use the policy and IPC retains the local-origin gate", () => {
  const createWindowModule = readFileSync(new URL("./main/create-window.mjs", import.meta.url), "utf8");
  ...
  assert.match(createWindowModule, /shell\.openExternal\(externalWebUrl\(url\)\)/);
});
