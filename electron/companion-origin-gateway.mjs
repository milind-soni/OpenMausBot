...
export function validCompanionOriginTarget(target, platform = process.platform) {
  ...
  if (platform === "win32") {
    return /^\\\\\.\\pipe\\astra-companion-origin-[1-9][0-9]*-[0-9a-f-]{36}$/i.test(
      target.socketPath,
    );
  }
  return (
...
export function createCompanionOriginEndpoint({
  ...
  if (platform === "win32") {
    return Object.freeze({
      pid: processId,
      socketPath: `\\\\.\\pipe\\astra-companion-origin-${processId}-${identifier()}`,
      directory: null,
    });
  }
    ...
    const outgoing = request(
      ...
      (response) => {
        ...
        response.on("end", () => {
          ...
          try {
            finish(JSON.parse(Buffer.concat(chunks).toString("utf8"))?.app === "astra");
          } catch {
            finish(false);
          }
        });
