...
The iOS app is a thin, native client for the Astra instance running on
...
The Mac must be running Astra and must not be asleep. Desktop
...
The sidecar keeps its device registry in `~/.astra/devices.json`. That is
...
to reach. Astra then places the computer's MagicDNS name in that
...
Tailscale is optional. The direct path does not use an Astra-operated
...
The Astra control plane stores account and installation metadata plus
...
The Mac must remain awake with Astra running for chats, approvals, and
...
An Astra account is not required for nearby, manual, or Tailscale
...
Passwords or any enabled third-party AutoFill provider. AstraMobile does
...
swift test
xcodegen generate
xcodebuild -project AstraCompanion.xcodeproj \
  -scheme AstraCompanion \
  -sdk iphonesimulator \
