...
Run the Astra server anywhere; connect from the desktop app, any
...
1. **Stable server identity.** The server generates `environmentId` once
   (`ASTRA_DATA_DIR/environment-id`) and serves a descriptor at
   `/.well-known/astra/environment` — id, label, platform, version,
...
3. **Managed tunnel**: the cloudflared managed-tunnel channel the companion
   ...
   Shipped for the command line as `astra login` + `serve --tunnel`
