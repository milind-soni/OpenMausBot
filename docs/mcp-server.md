...
The Astra desktop app includes a local stdio MCP server. It lets another MCP client coordinate your
Astra team while the desktop app and its harness are running.
...
Start Astra, then configure the MCP client to run:
...
```json
{
  "mcpServers": {
    "astra": {
      "command": "pnpm",
      "args": ["--dir", "/absolute/path/to/Astra", "mcp"]
    }
  }
}
```
...
1. In Astra, open **Settings → Phone → Set up a phone** and reveal the
...
```json
{
  "mcpServers": {
    "astra": {
      "command": "/Applications/Astra.app/Contents/MacOS/Astra",
      "args": ["/Applications/Astra.app/Contents/Resources/server/mcp-server.js"],
...
On Windows, use the installed `Astra.exe` as `command`, the adjacent
`resources\\server\\mcp-server.js` as the argument, and the same `ELECTRON_RUN_AS_NODE=1` environment value.
The usual per-user install is under `%LOCALAPPDATA%\\Programs\\Astra`.

On Ubuntu `.deb` installs, the executable is normally `/opt/Astra/astra` and the script is
`/opt/Astra/resources/server/mcp-server.js`. Use the same environment value.

## Connection discovery

With no configuration, the MCP process probes Astra's three desktop ports (`8799`, `18799`, and `28799`)
and accepts only a health response that identifies itself as Astra. This handles the desktop's normal
fallback when another local process already owns port 8799.

Set `ASTRA_PORT` to force one local port, or `ASTRA_URL` to use an explicit HTTP(S) origin. Cleartext remote
HTTP is rejected unless `ALLOW_INSECURE_HTTP=true`; HTTPS should be used outside loopback. An optional
`ASTRA_TOKEN` is sent as a bearer token for authenticated reverse proxies. When a token is set, an
explicit `ASTRA_URL` or `ASTRA_PORT` is required so the credential is never sent while probing unrelated
local ports. `ASTRA_MCP_TIMEOUT_MS` can set an HTTP timeout between 1,000 and 120,000 milliseconds.
In packaged builds, `ASTRA_TOKEN` is required for mutating tools as
