# Deploy Astra on a VPS

From a blank Linux server to Astra running on it around the clock, reachable from your laptop, the desktop app and your phone, with your bots working while every laptop is closed. It assumes nothing beyond being able to open a terminal and paste commands. About twenty minutes, most of it waiting.
...
|---|---|---|---|
| **A. Public address, no domain** (`serve --tunnel`) | an Astra account (email code) | anyone with a pairing code, over HTTPS | the fastest path; a phone on cellular |
...
Whichever you pick, the login is the same: you **pair** each device once with a short code and it stays signed in. A session lasts 30 days; using it with half that or less left renews it to a full 30, up to 180 days from pairing (`ASTRA_SESSION_TTL_DAYS` and `ASTRA_SESSION_MAX_DAYS` change both numbers). There is no password.
...
No domain, no proxy, no open port. The server gets an address like `https://c-7f3a9c.astra.com` through a Cloudflare tunnel; only traffic through the tunnel reaches it, and that traffic still has to pair.

```sh
npx astra setup          # once: choose AI access, connect, and choose a model
npx astra login          # once: an emailed code signs this machine in and reserves its address
npx astra serve --tunnel # runs the server there and prints the pairing link with a QR code
```

`setup` connects an AI provider; it is separate from the Astra account.
...
3. **Get the deploy files and set the name:**
   ...
   ```sh
   git clone https://github.com/milind-soni/Astra && cd Astra/deploy
   cp .env.example .env
...
In this path, every `npx astra …` command below is run inside the container instead:

```sh
docker compose exec omb node dist-server/astra.js pair --label "My MacBook"
```
...
```sh
sudo -H npx --yes astra browser install --with-deps
```

Then install the browser in the service account's home, from the `maus` shell:

```sh
npx --yes astra browser install
```
...
The npm Astra package does not install model engine CLIs. For paths A and C,
...
`~/.claude`), separately from Astra's `~/.astra`. Keep that home when
...
```sh
npx astra pair --label "Kitchen iPad"
```

```
pairing code:  RR8Y-BLR6-H939
expires:       10:59:45 AM (single use)
open or scan:  https://c-7f3a9c.astra.com/pair#code=RR8Y-BLR6-H939
```
...
```sh
npx astra sessions              # id, device, scope, last seen, expires
npx astra sessions revoke ID    # signs that device out and closes its stream at once
npx astra status                # what the server says about itself
```

## Keep it running

`npx astra serve` is a plain foreground process. For systemd (paths A and C),
install a chosen release first, from the `maus` shell. Replace `X.Y.Z` with the
published version you want to run:

```sh
npm install --global --prefix "$HOME/.local" astra@X.Y.Z
```

Stop the foreground server with Ctrl-C before enabling the service. From the
administrator shell, save this as `/etc/systemd/system/astra.service`:

```ini
# /etc/systemd/system/astra.service
[Unit]
Description=Astra server
...
ExecStart=/home/maus/.local/bin/astra serve --tunnel --no-pair
Restart=always
RestartSec=5
...
```sh
sudo systemctl daemon-reload && sudo systemctl enable --now astra
journalctl -u astra -f            # the server's log, including "tunnel: live at …"
```

Use `--tailscale` instead of `--tunnel` for path C. `--no-pair` skips printing a code at every restart; mint one with `npx astra pair` when you need it. Docker (path B) restarts on its own (`restart: unless-stopped`).
...
- **Paths A and C with the service above:** choose the new published version,
  ...
  sudo -iu maus npm install --global --prefix /home/maus/.local astra@X.Y.Z
  ...
  desired release as `npx --yes astra@X.Y.Z serve --tunnel`.
- **Path B:** `cd Astra/deploy && docker compose pull omb && docker compose up -d`.
...
process, or `sudo systemctl stop astra` from the administrator shell for
...
```sh
umask 077
backup_dir=$(mktemp -d "$PWD/astra-backup.XXXXXX")
tar czf "$backup_dir/astra-data.tgz" -C "$HOME" .astra
```
...
`Astra/deploy`, stop the app before archiving; `deploy_data` is the default
volume name, so use your actual volume name if you changed the Compose project:

```sh
docker compose stop omb
backup_dir=$(mktemp -d "$PWD/astra-backup.XXXXXX")
docker run --rm -v deploy_data:/data:ro -v "$backup_dir":/b alpine sh -c 'umask 077; tar czf /b/astra-data.tgz -C /data .'
```
...
service with `sudo systemctl start astra`, or `docker compose start omb`.
...
- The one thing a stranger can read is `/.well-known/astra/environment` (the server's id, label, version, capabilities) and `/api/health` (only the app name). Everything else answers "pair this device".

## Troubleshooting

**`serve --tunnel` says "no account on this machine yet".** Run `npx astra login` on this machine first; the credentials are per machine.

**The tunnel stays on "retrying".** The server is running and usable locally; the public hop is not verified yet. Wait a minute (Cloudflare needs a moment on a fresh address), then check `journalctl`/the terminal for the reason. If it never comes up, `npx astra logout && npx astra login` issues a fresh address.
...
**What does the server think it is?** `https://<address>/.well-known/astra/environment` is public and shows its id, label, version and capabilities; `npx astra status` prints the same on the server.

**Something else.** `journalctl -u astra --since -10m` (or `docker compose logs omb --tail 100`) shows the server's startup lines. Paste them with your question in the community channel.
...
```sh
sudo install -d -o root -g root -m 0755 /opt/astra-browser
sudo cp -R /home/maus/.agent-browser/browsers/chrome-VERSION /opt/astra-browser/
sudo chown -R root:root /opt/astra-browser/chrome-VERSION
sudo chmod -R go-w /opt/astra-browser/chrome-VERSION
```

Save this as the root-owned `/etc/apparmor.d/astra-chrome`, replacing
...
profile astra-chrome /opt/astra-browser/chrome-VERSION/chrome flags=(unconfined) {
  userns,
}
```
...
```sh
sudo apparmor_parser -r /etc/apparmor.d/astra-chrome
```
...
```ini
Environment=AGENT_BROWSER_EXECUTABLE_PATH=/opt/astra-browser/chrome-VERSION/chrome
```
