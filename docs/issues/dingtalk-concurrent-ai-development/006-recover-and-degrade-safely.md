...
- Schema v6 persists the singleton instance fence, node/run runtime lease state, outbox delivery claims, Provider circuits, retention deadlines, and degradation state.
...
- Focused regression: 18 test files / 81 tests passed, including all collaboration tests and `server/message-db.test.ts`.
