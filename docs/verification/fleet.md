...
privileges, read-only system, the `${ASTRA_PORT}`-style expansion), the
...
```sh
astra fleet init --domain example.test --dry-run
astra fleet create acme --admin owner@acme.test --dry-run
```
...
1. Fresh Ubuntu 24.04 or newer, Caddy from apt, `npm install -g astra`,
   Claude Code installed once as root, a wildcard DNS record at the server.
2. `astra fleet init --domain <domain>`; check `systemctl status
   astra-fence` and `nft list table inet astra`.
3. `astra fleet create alpha --admin you@example.test --cap 1` and the
...
4. Isolation: as `omb-alpha` (`runuser -u omb-alpha -- curl -s
   ...
   as root it must answer. `ls /var/lib/astra/beta` as `omb-alpha` must
   be denied.
