...
OpenMausBot-owned credential recovery is retained privately alongside it. Keep
these copies until the restored workspace is checked. Do not publish them.
The downloaded archive is encrypted; private staging and safety directories
are not. They can contain credential metadata and require normal host/disk
...
settings and credentials, browser-state allowlisting, and the automatic safety
copy. It sends a new turn through the restored conversation. Wrong passwords,
wrong confirmation, non-admin access and post-confirmation writes are rejected.
...
pnpm exec vitest run server/workspace-backup.test.ts \
  server/workspace-backup-startup.test.ts \
...
node --test electron/workspace-backup-credentials.node-test.mjs
...
cross-directory path repair, interrupted replacement, credential finalization
failure and recovery. The maintenance checks keep ordinary requests, turns,
schedulers and the separate webhook receiver from writing during a snapshot.
Unknown external writers cannot be frozen: users must stop external editors and
managed desktops before exporting.

Credential tests use fake private ports and an isolated store, never the user's
keychain. They check scope, serialization, rollback, headless persistence and
desktop migration. The renderer recipe drives the real Settings screen in an
isolated Chromium fixture with simulated backup endpoints; it checks the native
...
Backups include this workspace's durable files, complete message records and
OpenMausBot-managed keys—not remote VM disks, other workspaces, external project
files, external CLI/browser login homes, or live device sessions. The 10 GB and
100,000-entry limits fail visibly. Scheduled routines/webhooks are paused and
...
external providers, phone recovery, OS-specific file-picker appearance, or a
real Electron keychain transfer. No such live mutation is part of this recipe.
