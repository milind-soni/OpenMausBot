...
- stable `Astra-amd64.deb` and `Astra.AppImage` copies used by the latest-download links;
...
[Astra releases](https://github.com/milind-soni/Astra/releases). Then verify the checksum
...
Data lives in `~/.astra/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.astra`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.astra/config.json` via `PUT /api/config` and the API
...
Astra release repository, and never change the upstream publish target in a feature PR unless
