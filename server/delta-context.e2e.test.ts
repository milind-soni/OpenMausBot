...
it("gives a delegate_bot source today's fresh session and replay when its soul changed since the session started", () => fixture(async (f) => {
  ...
  const replies = async () => (await f.messages(threadId)).filter((m: any) => /^@(QA|Ops) replied to the delegated task/.test(m.text ?? "")).length;
  await expect.poll(replies, { timeout: 30_000 }).toBe(2);
  // The wake turn's dispatch-time soul read happens before its CLI launch,
  // and the edit below must land after that read (while the gated turn
  // holds) so the fresh session is the third turn's. Waiting for the
  // launch keeps that true on machines where the Local VM probe slows
  // setup — the launch record is written at process start, gate or not.
