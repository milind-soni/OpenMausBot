...
export interface LocalVmTurnPrepDeps {
  lateBound: {
    ...
    getLocalVmProvisionBusy(): boolean;
...
export function createLocalVmTurnPrep(deps: LocalVmTurnPrepDeps) {
  const { broadcast, getLocalVmProvisionBusy, setLocalVmProvisionBusy } = deps.lateBound;
  const {
  ...
  async function readyLocalVmForTurn(botId: string, target: LocalVmTarget, isCurrent = () => true) {
    ...
    const ownsProvision = !getLocalVmProvisionBusy();
    ...
    try {
      ...
      try {
        ...
        // which is more use to the person than "podman run exited
        // non-zero".
