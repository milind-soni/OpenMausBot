...
import { astraStatusSystemPrompt, readAstraStatus } from "./astra-status-capsule.ts";
...
posixOnly("readAstraStatus", () => {
  it("projects only fresh normalized two-VM capability data", () => {
    const capsule = successCapsule();
    // Cross-language receipt produced by scripts/aos_astra_status.py
    ...
    const status = readAstraStatus({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
    ...
    const prompt = astraStatusSystemPrompt({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
  ...
  it("rejects a signed success capsule whose installed and accepted hashes differ", () => {
    ...
    expect(readAstraStatus({ cachePath: path, now: NOW })).toMatchObject({
      freshness: "unknown",
      reason: "invalid",
  ...
  it("turns stale, future-skewed, failed, and receipt-tampered state into unknown", () => {
    const path = cachePath(successCapsule());
    expect(readAstraStatus({ cachePath: path, now: new Date("2026-08-22T06:35:00Z") })).toMatchObject({
      freshness: "stale", reason: "stale", runtimeState: "unknown", readyCount: 0,
      ui: { twoUp: false },
    });
    expect(readAstraStatus({ cachePath: path, now: new Date("2026-08-22T06:29:59Z") })).toMatchObject({
      freshness: "unknown", reason: "clock_skew", runtimeState: "unknown", readyCount: 0,
    });

    const failurePath = cachePath(failedCapsule());
    expect(readAstraStatus({ cachePath: failurePath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
    ...
    expect(readAstraStatus({ cachePath: tamperedPath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
  ...
  it("rejects signed extra fields, insecure modes, and same-path symlinks", () => {
    ...
    expect(readAstraStatus({ cachePath: extraPath, now: NOW }).reason).toBe("invalid");

    const insecurePath = cachePath(successCapsule());
    chmodSync(insecurePath, 0o644);
    expect(readAstraStatus({ cachePath: insecurePath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkPath = join(dirname(targetPath), "latest-link.json");
    symlinkSync(targetPath, linkPath);
    expect(readAstraStatus({ cachePath: linkPath, now: NOW }).reason).toBe("missing_or_insecure");
  });
  ...
  it("rejects a non-0700 parent and a symlinked parent", () => {
    ...
    expect(readAstraStatus({ cachePath: looseParentPath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkRoot = mkdtempSync(join(tmpdir(), "openmaus-status-parent-link-"));
    roots.push(linkRoot);
    const linkedParent = join(linkRoot, "astra");
    symlinkSync(dirname(targetPath), linkedParent, "dir");
    expect(
      readAstraStatus({ cachePath: join(linkedParent, "latest.json"), now: NOW }).reason,
...
it.skipIf(process.getuid !== undefined)(
  ...
  () => {
    ...
    expect(astraStatusSystemPrompt({ cachePath: path, now: NOW })).toContain(
      "runtime_state=unknown",
    );
  },
);
