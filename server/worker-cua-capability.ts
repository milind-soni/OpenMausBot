...
/** The exact platform file manager admitted by a desktop capability. Exported
 * so the approval card can name the same executable without duplicating the
 * security-sensitive table. */
export function workerFileManager(platform: WorkerPlatform): string {
  return FILE_MANAGER[platform];
}
...
export const WORKER_BROWSER_CUA_TOOLS = [
  "start_session",
  "end_session",
  "list_windows",
  "browser_prepare",
...
export const WORKER_DESKTOP_CUA_TOOLS = [
  "start_session",
  "end_session",
  "launch_app",
  "list_windows",
...
/** Required only for macOS readiness. It is deliberately absent from the
 * agent-facing tool lists above; the control plane invokes it with
 * `prompt:false`, and the stable base policy refuses every other shape. */
const macHealthTools = (platform: WorkerPlatform): readonly string[] =>
  platform === "macos" ? ["check_permissions"] : [];

/** The control plane repeats the native manifest's tool ceiling before it
 * invokes the one-shot CUA client. The daemon remains the final authority,
 * but rejecting a mismatched surface here avoids even sending it to SSH. */
export function workerCuaToolsForSurface(surface: WorkerTaskManifest["surface"]): readonly string[] {
  return surface === "browser" ? WORKER_BROWSER_CUA_TOOLS : WORKER_DESKTOP_CUA_TOOLS;
}
...
export function workerCuaCapabilityManifest(
  ...
  if (manifest.surface === "browser") {
    ...
    return [
      ...head,
      ...macHealthTools(manifest.platform).map((tool) => `    - ${tool}`),
      ...WORKER_BROWSER_CUA_TOOLS.map((tool) => `    - ${tool}`),
      "",
      "resources:",
      "  apps:",
  ...
  return [
    ...head,
    ...macHealthTools(manifest.platform).map((tool) => `    - ${tool}`),
    ...WORKER_DESKTOP_CUA_TOOLS.map((tool) => `    - ${tool}`),
    "",
    "resources:",
    "  apps:",
    ...app(manifest.target.ideExecutable),
    ...app(workerFileManager(manifest.platform)),
    "  files:",
    "    read:",
