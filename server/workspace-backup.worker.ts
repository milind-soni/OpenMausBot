...
const { dataDir, options } = workerData as { dataDir: string; options: CreateWorkspaceBackupOptions };
try {
  // Same snapshot and cleanup implementation; never send password/options back.
  parentPort!.postMessage({ result: await createWorkspaceBackupSnapshot(dataDir, options) });
} catch (error) {
