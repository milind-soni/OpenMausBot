...
const SCHEMA = "aos.astra_status.v1";
...
export const OPENMAUS_STATUS_CACHE_PATH = join(
  homedir(),
  ".local/state/aos-session-bridge/astra/latest.json",
);

type AstraSlot = z.output<typeof slotSchema>;
type AstraUi = z.output<typeof uiSchema>;
type AstraCapsule = z.output<typeof capsuleSchema>;

export interface AstraStatusDigest {
...
export interface AstraStatusReadOptions {
  cachePath?: string;
  now?: Date;
}
...
function expectedUi(twoUp: boolean): AstraUi {
  return {
    two_up: twoUp,
...
function sameUi(left: AstraUi, right: AstraUi): boolean {
  return (
...
function unsignedDocument(capsule: AstraCapsule): JsonObject {
...
function validSlotState(slot: AstraSlot): boolean {
  return (
...
function validateCapsule(value: JsonValue): AstraCapsule | null {
...
function unknownDigest(
  reason: NonNullable<AstraStatusDigest["reason"]>,
  freshness: AstraStatusDigest["freshness"] = "unknown",
  receipt?: Pick<AstraStatusDigest, "observedAt" | "receiptSha256">,
): AstraStatusDigest {
  return {
    schema: SCHEMA,
    freshness,
...
export function readAstraStatus(
  ...
  const result: AstraStatusDigest = {
    schema: SCHEMA,
    freshness: "fresh",
    ...receipt,
...
export function astraStatusSystemPrompt(options: AstraStatusReadOptions = {}): string {
  const status = readAstraStatus(options);
  const receipt = [
