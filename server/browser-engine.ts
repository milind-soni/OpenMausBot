...
function packagedBrowser(options: BrowserLookupOptions) {
  const resources = (options.env ?? process.env).ASTRA_RESOURCES_PATH;
...
/** ASTRA_AGENT_BROWSER_PATH, then the complete desktop bundle, pinned download, then
...
export function resolveAgentBrowserBinary(options: BrowserLookupOptions = {}): string | null {
  ...
  const override = env.ASTRA_AGENT_BROWSER_PATH?.trim();
...
export function browserEngineStatus(options: BrowserLookupOptions = {}): BrowserEngineStatus {
  ...
  if (bundle && (options.exists ?? existsSync)(bundle.directory)) {
    return { kind: "unavailable", reason: "The desktop browser bundle is incomplete. Reinstall or update Astra to repair it.", installable: false };
  }
  const platform = options.platform ?? process.platform;
...
export function agentBrowserFrame(input: {
  ...
  const file = join(tmpdir(), `astra-browser-${randomUUID()}.png`);
