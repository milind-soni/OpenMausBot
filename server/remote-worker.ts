...
export type RemoteWorkerErrorCode =
  ...
  | "worker_permission_attribution_mismatch"
...
export class RemoteWorkerLease {
  ...
  current(sshAlias: string, isBotBusy: (botId: string) => boolean, now = Date.now()): RemoteWorkerLeaseRecord | null {
    const key = sshAlias.toLowerCase();
    ...
    // for the whole TTL.
    if (record && (record.expiresAt <= now || !isBotBusy(record.botId))) this.records.delete(key);
  ...
  claim(
    ...
    this.records.set(sshAlias.toLowerCase(), { sshAlias, threadId, botId, expiresAt: now + this.ttlMs });
    return true;
  }
  ...
  releaseAlias(sshAlias: string): void {
    this.records.delete(sshAlias.toLowerCase());
  }
}
...
const healthReportSchema = z.object({
  ...
  permissionAttribution: z.enum(["driver-daemon", "host", "caller", "unknown"])
    .optional().catch(undefined),
  permissionProbeExpired: z.boolean().optional().catch(undefined),
...
export interface RemoteWorkerStatus {
  ...
  /** The parked manifest normally, or the exact approved task capability
   * while one is active. A merely well-formed digest is never sufficient. */
  expectedCapabilityDigest: string | null;
  capabilityMatches: boolean;
  permissionAttribution: "driver-daemon" | "host" | "caller" | "unknown";
  permissionProbeExpired: boolean;
...
/** Device-facing status. Readiness details such as channel paths, policy and
 * capability digests, and lease ownership stay inside the control plane. */
export interface PublicRemoteWorkerStatus {
  workerId: string;
  platform: WorkerPlatform;
  displayName: string;
  configured: boolean;
...
export function publicWorkerStatus(status: RemoteWorkerStatus): PublicRemoteWorkerStatus {
  return {
    ...
    lease: status.lease ? { expiresAt: status.lease.expiresAt } : null,
...
export function defaultRemoteWorkerRunner(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => finish(() => {
      ...
      else reject(new Error("worker SSH command failed"));
    }));
  });
}
...
/** A probe may report only the channel installed by the documented daemon
 * setup. Syntactic safety alone would still allow redirecting the bridge to
 * an unrelated local socket on the worker. */
export function isExpectedWorkerChannelPath(platform: WorkerPlatform, value: string): boolean {
  if (!isSafeChannelPath(value)) return false;
  if (platform === "windows") return value.toLowerCase() === "\\\\.\\pipe\\cua-driver";
  return /^\/Users\/[A-Za-z0-9._-]+\/\.openmausbot\/run\/cua\.sock$/.test(value);
}
...
export function baseWorkerStatus(worker: ResolvedWorker): RemoteWorkerStatus {
  return {
    ...
    expectedCapabilityDigest: worker.expectedParkedCapabilitySha256,
    capabilityMatches: false,
    permissionAttribution: "unknown",
    permissionProbeExpired: false,
    accessibilityGranted: null,
    screenRecordingGranted: null,
    lease: null,
    errorCode: "worker_unconfigured",
    problem: "Configure this worker's SSH alias and expected base-policy and parked-capability SHA-256 digests",
  };
}
...
export function applyHealthReport(status: RemoteWorkerStatus, raw: JsonValue): RemoteWorkerStatus {
  ...
  status.capabilityMatches = status.capabilityLoaded
    && status.capabilityDigest !== null
    && status.capabilityDigest === status.expectedCapabilityDigest;
  status.permissionAttribution = report.permissionAttribution ?? "unknown";
  status.permissionProbeExpired = report.permissionProbeExpired === true;
...
export function evaluateSharedHealth(status: RemoteWorkerStatus): RemoteWorkerStatus | null {
  ...
  if (!isExpectedWorkerChannelPath(status.platform, status.channelPath)) {
    return failWorker(status, "no_interactive_session", "worker_channel_missing",
      "The worker reported a CUA control channel outside the fixed worker path");
  }
  ...
  if (!status.capabilityMatches) {
    return failWorker(status, "policy_mismatch", "worker_capability_mismatch",
      "The active CUA capability digest does not match the parked or approved task capability");
  }
  return null;
}
...
export function remoteWorkerMcp(
  ...
  // Revalidate the transport identity even though resolved configuration has
  // already passed the schema; descriptors are also constructed directly in
  // tests and must never carry arbitrary SSH options.
  remoteWorkerSshBaseArgs(worker.sshAlias);
  return {
    command: SPAWNED_PROXIES.workerMcp,
    // The worker-only MCP process talks to these loopback endpoints. It does
    // not spawn the long-lived remote MCP proxy: approved CUA calls are
    // one-shot operations owned by WorkerTaskService, keeping the shared
    // Local VM/VPS/IDE/CLI bridge completely out of this path.
    args: [],
