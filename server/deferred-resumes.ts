...
// provided or dismissed. index.ts wires createDeferredResumes just before
// createGroupTurn, the earliest module-level by-value consumer of its
// drains; runGroupMemberTurn and startTurn are wrapper thunks over consts
// index.ts declares after that site.
 ...
 * holds wrapper thunks for the functions index.ts binds after the factory is
 * wired (runGroupMemberTurn comes from createGroupTurn just below the wiring
 * site, startTurn from createStartTurn further down); the helpers are
 * hoisted function declarations, safe to pass by value, and groupQueues is a
 * const declared above the wiring site. */
