...
describe("harness HTTP API", () => {
  ...
  it("keeps Teach a skill off by default and persists an explicit opt-in", async () => {
    const before = await api("GET", "/api/config");
    expect(before.status).toBe(200);
    expect(before.body.features).toEqual({ browser: false, skillRecorder: false, showToolCalls: false, menuBar: false });

    const saved = await api("PATCH", "/api/config", {
      features: { skillRecorder: true },
    });
    expect(saved.status).toBe(200);
    expect(saved.body.features).toEqual({ browser: false, skillRecorder: true, showToolCalls: false, menuBar: false });
    ...
    expect(tools.body.features).toEqual({ browser: false, skillRecorder: true, showToolCalls: true, menuBar: false });
