...
describe("staging", () => {
  ...
  it("does not send bytes from a path swapped after the preflight", async () => {
    ...
    const runner = async (_args: string[], options: WorkerTaskStreamOptions) => {
      const source = join(root, "src", "main.txt");
      const replacement = join(root, "replacement.txt");
      writeFileSync(replacement, "private replacement bytes");
      rmSync(source);
      ...
      return {
        stdout: Buffer.from('{"ok":true,"version":1,"op":"stage","files":1}\n'),
        stderr: "",
      };
    };
...
describe("validate", () => {
  ...
  it("refuses a worker holding a different command list", async () => {
    const fake = fakeCompanion([
      { ok: true, version: 1, op: "validate", taskRoot, files: 0, commandIds: ["other"] },
    ]);
    await expect(validateWorkerTask(worker, manifest, digest, fake.runner))
      .rejects.toThrow(/different set of task commands/);
  });

  it("refuses a success reply from another companion protocol version", async () => {
    const fake = fakeCompanion([
      { ok: true, version: 2, op: "validate", taskRoot, files: 0, commandIds: ["build"] },
    ]);
    await expect(validateWorkerTask(worker, manifest, digest, fake.runner))
...
describe("approved computer calls", () => {
  it("describes only a fixed tool name", async () => {
    const fake = fakeCompanion([]);
    fake.runner = ((args: string[]) => {
      fake.calls.push({ args, stdin: "" });
      return Promise.resolve({ stdout: '{"type":"object"}\n', stderr: "" });
    }) as typeof fake.runner;
    await expect(describeWorkerComputerTool(worker, "click", fake.runner)).resolves.toContain("object");
    expect(fake.calls[0].args.slice(-3)).toEqual(["cua-driver", "describe", "click"]);
  });

  it("sends tool arguments on stdin and never in SSH argv", async () => {
    const stream = fakeStream(Buffer.from('{"content":[{"type":"text","text":"clicked"}]}\n'));
    const channel = worker.platform === "windows"
      ? "\\\\.\\pipe\\cua-driver"
      : "/Users/worker/.openmausbot/run/cua.sock";
    await callWorkerComputerTool(worker, channel, "click", { x: 12, y: 30 }, stream.runner);
    expect(stream.calls[0].slice(-5)).toEqual(["cua-driver", "call", "click", "--socket", channel]);
    expect(stream.calls[0].join(" ")).not.toContain('"x"');
    expect(stream.bytes().toString("utf8")).toBe('{"x":12,"y":30}\n');
  });

  it("refuses a non-worker channel before starting SSH", async () => {
    const stream = fakeStream(Buffer.alloc(0));
    await expect(callWorkerComputerTool(worker, "/tmp/cua.sock", "click", {}, stream.runner))
      .rejects.toThrow(/control channel/);
    expect(stream.calls).toHaveLength(0);
  });
});
...
describe("reset", () => {
  it("names the pinned base policy so the worker proves what it parked on", async () => {
    const fake = fakeCompanion([{ ok: true, version: 1, op: "reset", capabilitySha256: worker.expectedParkedCapabilitySha256! }]);
  ...
  it("refuses a reset that reports a different parked capability", async () => {
    const fake = fakeCompanion([{ ok: true, version: 1, op: "reset", capabilitySha256: "d".repeat(64) }]);
    await expect(resetWorkerTask(worker, "task-1", fake.runner)).rejects.toThrow(/different parked capability/);
  });
});

describe("parked refresh", () => {
  it("names only the pinned base policy and requires the parked digest back", async () => {
    const fake = fakeCompanion([{
      ok: true,
      version: 1,
      op: "resume",
      paused: false,
      capabilitySha256: worker.expectedParkedCapabilitySha256!,
    }]);
    await expect(refreshParkedWorker(worker, fake.runner))
      .resolves.toBe(worker.expectedParkedCapabilitySha256);
    expect(JSON.parse(fake.calls[0].stdin)).toEqual({
      op: "resume",
      expectedBasePolicySha256: worker.expectedBasePolicySha256,
    });
  ...
  it("refuses a refresh that returns a different parked digest", async () => {
    const fake = fakeCompanion([{
      ok: true,
      version: 1,
      op: "resume",
    ...
    await expect(refreshParkedWorker(worker, fake.runner))
      .rejects.toThrow(/different parked capability/);
  });
});
