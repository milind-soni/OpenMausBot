...
/** Per-platform defaults. A fresh worker needs an alias plus both stable
 * policy digests; application paths and profile labels have conventional
 * values the operator can override when their install differs. */
...
const workerConfigSchema = z.object({
  ...
  expectedDriverVersion: z.string().max(32).refine((value) => value === "" || value === WORKER_DRIVER_VERSION, {
    message: `must be exactly CUA Driver ${WORKER_DRIVER_VERSION}`,
  }).optional(),
  expectedBasePolicySha256: z.string().refine((value) => value === "" || SHA256.test(value), {
    message: "must be a SHA-256 digest",
  }).optional(),
  expectedParkedCapabilitySha256: z.string().refine((value) => value === "" || SHA256.test(value), {
    message: "must be a SHA-256 digest",
  }).optional(),
...
export const workerConfigMapSchema = z
  ...
  .refine(
    (workers) => {
      // OpenSSH host-pattern matching is case-insensitive. Treat aliases that
      // differ only by case as one machine or they would receive independent
      // input leases for the same desktop.
      const aliases = Object.values(workers).map((worker) => worker.sshAlias.toLowerCase());
...
export interface ResolvedWorker {
  ...
  expectedParkedCapabilitySha256: string | null;
...
export function resolveWorker(id: string, raw: WorkerConfig): ResolvedWorker {
  ...
  const digest = raw.expectedBasePolicySha256 && SHA256.test(raw.expectedBasePolicySha256)
    ...
    : null;
  const parkedDigest = raw.expectedParkedCapabilitySha256 && SHA256.test(raw.expectedParkedCapabilitySha256)
    ? raw.expectedParkedCapabilitySha256.toLowerCase()
    : null;
  return {
    id,
    ...
    expectedParkedCapabilitySha256: parkedDigest,
    ...
    configured: digest !== null && parkedDigest !== null,
  };
}
