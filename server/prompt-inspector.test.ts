...
describe("provider prompt captures", () => {
  ...
  it("redacts credentials before writing and never retains unrecognized response headers", () => {
    const dir = directory(), store = new PromptInspector(dir);
  ...
  it("bounds retention and bodies without altering the observed input", () => {
    const store = new PromptInspector(directory()), body = { text: "x".repeat(4 * 1024 * 1024) };
  ...
  it("does not revive deleted captures and removes cross-thread copies", () => {
    const store = new PromptInspector(directory());
  ...
  it("marks interrupted requests after a restart and rejects traversal", () => {
    const dir = directory(), store = new PromptInspector(dir); store.capture(meta, "agent-input", {});
    expect(store.read("thread")[0].status).toBe("sending"); expect(new PromptInspector(dir).read("thread")[0].status).toBe("interrupted");
  ...
  it("captures real HTTP request bodies, separates call usage from turn totals, and keeps auth off disk", async () => {
    const requests: unknown[] = [], store = configurePromptInspector(directory());
...
it("isolates concurrent turns, ignores stale events, and never labels running thread totals as turn usage", async () => {
  const store = configurePromptInspector(directory()), fake = makeFakeDriver({ kind: "acp-fixture" });
...
it("preserves provider failures and operates when capture storage cannot be written", async () => {
  const dir = directory(), store = configurePromptInspector(dir), fake = makeFakeDriver();
  const instance = await fake.driver.create({ instanceId: "fixture", displayName: undefined, enabled: true, environment: {}, config: {} });
