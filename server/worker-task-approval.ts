...
import { workerFileManager } from "./worker-cua-capability.ts";
...
/** Refuse an approval that the card cannot show in full. Silently truncating
 * would turn an exact-manifest decision into approval of hidden work. */
export const WORKER_TASK_APPROVAL_MAX_CHARS = 64 * 1024;
...
export function describeWorkerTask(
  ...
  const applications = manifest.surface === "browser"
    ? [
        `Browser executable: ${JSON.stringify(manifest.target.browserExecutable)}`,
        `Intended browser profile (verify the selected window): ${JSON.stringify(manifest.target.browserProfile)}`,
      ]
    : [
        `IDE executable: ${JSON.stringify(manifest.target.ideExecutable)}`,
        `File manager executable: ${JSON.stringify(workerFileManager(manifest.platform))}`,
      ];
  const lines = [
    `Worker: ${shown.displayName} (${os})`,
    ...
    "",
    `Input files (${manifest.files.length}, ${humanBytes(bytes)} total):`,
    ...(manifest.files.length > 0
      ? manifest.files.map((file) =>
          `• ${JSON.stringify(file.path)} · ${file.size} bytes · SHA-256 ${file.sha256}`)
      : ["• None"]),
    "",
    `Commands (${manifest.commands.length}):`,
    ...manifest.commands.map((command) => `• ${JSON.stringify({
      id: command.id,
  ...
  if (manifest.origins.length > 0) {
    lines.push("", "Browser origins:", ...manifest.origins.map((origin) => `• ${JSON.stringify(origin)}`));
  }
  lines.push(
    "",
    "Result paths:",
    ...manifest.resultPaths.map((path) => `• ${JSON.stringify(path)}`),
    "",
    `Expires: ${new Date(manifest.expiresAt).toISOString()} · idles out after ${
      Math.round(manifest.idleTimeoutMs / 60_000)
    } min`,
  );
  const description = lines.join("\n");
  if (description.length > WORKER_TASK_APPROVAL_MAX_CHARS) {
    throw new Error(
      `Worker task approval details exceed ${WORKER_TASK_APPROVAL_MAX_CHARS} characters; split the task`,
    );
  }
  return description;
}
...
function pushApprovalCard(
  ...
  return bus.store.appendMessage(threadId, {
    ...
    card: {
      ...
      subtitle: describeWorkerTask(worker, manifest, digest),
      options: ["Allow", "Deny"],
      requestId,
