...
export interface WorkerStatusOptions {
  ...
  capabilityDigestForWorker?: (workerId: string) => string | null;
}

export function workerStatus(worker: ResolvedWorker, options: WorkerStatusOptions = {}): Promise<RemoteWorkerStatus> {
  const expectedCapabilityDigest = options.expectedCapabilityDigest
    ?? options.capabilityDigestForWorker?.(worker.id)
    ?? worker.expectedParkedCapabilitySha256;
  const resolved = { ...options, expectedCapabilityDigest };
  return worker.platform === "windows" ? windowsWorkerStatus(worker, resolved) : macWorkerStatus(worker, resolved);
}
