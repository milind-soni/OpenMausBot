...
export interface ClaudeStreamDeps {
  ...
  /** per-thread retry bookkeeping; `rebuilt` marks a session the driver
   * relaunched from recoveryText rather than an accepted resume cursor */
  retry: { attempt: number; cancelled: boolean; rebuilt?: boolean };
}

export function handleLine(line: string, deps: ClaudeStreamDeps): void {
  const { session, threadId, settle, emit, base, currentTurnId, retry } = deps;
