...
const roomPostsThisTurn = createTurnScopedCounter();
...
export const handlers = {
  ...
  async coordinate_bots(args: Json, ctx: ToolContext): Promise<ToolOutcome> {
    ...
    const usable = Array.isArray(ids) && ids.length > 0 && ids.length <= 4
  ...
  async post_to_room(args: Json, ctx: ToolContext): Promise<ToolOutcome> {
    ...
    if (roomPostsThisTurn(ctx.turnGeneration) >= MAX_ROOM_POSTS_PER_TURN) {
      return {
    ...
    roomPostsThisTurn(ctx.turnGeneration, 1);
    return {
