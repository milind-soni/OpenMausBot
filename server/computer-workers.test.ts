...
const parked = "b".repeat(64);

const twoWorkers = {
  "mac-guest": { platform: "macos", sshAlias: "macguest", expectedBasePolicySha256: policy, expectedParkedCapabilitySha256: parked },
  "win-box": { platform: "windows", sshAlias: "winbox", expectedBasePolicySha256: policy, expectedParkedCapabilitySha256: parked },
} as const;
...
describe("worker registry", () => {
  ...
  it("rejects two workers pointed at one machine", () => {
    ...
    const parsed = workerConfigMapSchema.safeParse({
      "mac-a": { platform: "macos", sshAlias: "macguest", expectedBasePolicySha256: policy, expectedParkedCapabilitySha256: parked },
      "mac-b": { platform: "macos", sshAlias: "macguest", expectedBasePolicySha256: policy, expectedParkedCapabilitySha256: parked },
    });
    expect(parsed.success).toBe(false);
    expect(JSON.stringify(parsed.error?.issues)).toContain("distinct SSH alias");
  });

  it("treats SSH aliases as case-insensitive when enforcing uniqueness", () => {
    const parsed = workerConfigMapSchema.safeParse({
      "mac-a": { platform: "macos", sshAlias: "MacGuest", expectedBasePolicySha256: policy, expectedParkedCapabilitySha256: parked },
      "mac-b": { platform: "macos", sshAlias: "macguest", expectedBasePolicySha256: policy, expectedParkedCapabilitySha256: parked },
    });
  ...
  it("rejects an alias that could smuggle extra ssh arguments", () => {
    const parsed = workerConfigMapSchema.safeParse({
      evil: { platform: "macos", sshAlias: "host -o ProxyCommand=curl", expectedBasePolicySha256: policy, expectedParkedCapabilitySha256: parked },
    });
    expect(parsed.success).toBe(false);
  });
  ...
  it("holds a worker unconfigured until its base policy is pinned", () => {
    ...
    expect(workers[0].expectedParkedCapabilitySha256).toBeNull();
  });
  ...
  it("rejects a POSIX executable configured on a Windows worker", () => {
    const parsed = workerConfigMapSchema.safeParse({
      "win-box": {
        ...
        expectedParkedCapabilitySha256: parked,
  ...
  it("rejects an operator override to a different driver version", () => {
    expect(workerConfigMapSchema.safeParse({
      "mac-guest": {
        platform: "macos",
        sshAlias: "macguest",
        expectedDriverVersion: "99.0.0",
        expectedBasePolicySha256: policy,
        expectedParkedCapabilitySha256: parked,
      },
    }).success).toBe(false);
