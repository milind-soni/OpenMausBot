...
describe.each(["windows", "macos"] as const)("%s CUA capability", (platform) => {
  ...
  it("keeps the macOS daemon TCC health read available without exposing it on Windows", () => {
    if (platform === "macos") expect(capability(platform)).toContain("- check_permissions");
    else expect(capability(platform)).not.toContain("- check_permissions");
    expect(workerCuaToolsForSurface("browser")).not.toContain("check_permissions");
    expect(workerCuaToolsForSurface("desktop")).not.toContain("check_permissions");
  });
