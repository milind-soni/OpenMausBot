...
describe("DeepSeek Harness settings validation", () => {
  it.each([
    ["paired-device-unauthorized", "paired-device-unauthorized"],
    ["paired-model-catalog-unavailable", "paired-plugin-update-required"],
    ["provider-not-eligible", "provider-not-eligible"],
    ["model-update-conflict", "model-update-conflict"],
    ["dsh-management-failed", "host-unavailable"],
  ] as const)("maps internal %s failures to a stable public code and bounded copy", (internalCode, publicCode) => {
    const result = publicDeepSeekHarnessSettingsError(new DeepSeekHarnessSettingsError(
      409,
      internalCode,
      "upstream api_key=must-not-cross-the-wire",
    ));

    expect(result).toMatchObject({ status: 409, code: publicCode });
    expect(result.error.length).toBeLessThanOrEqual(160);
    expect(JSON.stringify(result)).not.toMatch(/api_key|must-not-cross-the-wire/i);
  });
  ...
  ] as const)("keeps %s recovery distinct at the public boundary", (code, copy) => {
    expect(publicDeepSeekHarnessSettingsError(
      new DeepSeekHarnessSettingsError(400, code, "upstream pairing detail"),
    )).toEqual({ status: 400, code, error: copy });
  });
