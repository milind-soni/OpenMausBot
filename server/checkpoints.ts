...
const EXCLUDES = `# Astra checkpoint excludes — never snapshotted, never removed by restore
.git/
.svn/
.hg/
node_modules/
...
function gitEnv(shadow: string, cwd: string): NodeJS.ProcessEnv {
  ...
  env.GIT_AUTHOR_NAME = "Astra Checkpoint";
  env.GIT_AUTHOR_EMAIL = "checkpoint@astra.local";
  env.GIT_COMMITTER_NAME = "Astra Checkpoint";
  env.GIT_COMMITTER_EMAIL = "checkpoint@astra.local";
  return env;
}
