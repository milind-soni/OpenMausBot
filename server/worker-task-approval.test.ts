...
describe("the approval card", () => {
  it("says which machine, what surface, and exactly what will run", () => {
    const exact = parsedManifest(HOST_TASK_PLATFORM, {
      files: [{ path: "src/main.ts", size: 12, sha256: "d".repeat(64) }],
      commands: [{
        id: "build",
        executable: manifest.commands[0].executable,
        argv: ["argument with spaces", "--check"],
        cwd: "src",
        timeoutMs: 42_000,
      }],
      resultPaths: ["result.json", "changes.patch", "reports/build.txt"],
    });
    const exactDigest = workerTaskManifestDigest(exact);
    const text = describeWorkerTask(worker, exact, exactDigest);
    ...
    expect(text).toContain(JSON.stringify({
      id: "build",
      executable: exact.commands[0].executable,
      argv: ["argument with spaces", "--check"],
      cwd: "src",
      timeoutMs: 42_000,
    }));
    expect(text).toContain('"reports/build.txt"');
    expect(text).toContain(`Manifest SHA-256: ${exactDigest}`);
  ...
  it("refuses a task whose exact approval details cannot fit on the card", () => {
    const oversized = parsedManifest(HOST_TASK_PLATFORM, {
      commands: [{
        id: "build",
        executable: manifest.commands[0].executable,
        argv: Array.from({ length: 20 }, () => "x".repeat(4_096)),
        cwd: "src",
        timeoutMs: 60_000,
      }],
    });
    expect(() => describeWorkerTask(worker, oversized)).toThrow(/approval details exceed/);
  });
