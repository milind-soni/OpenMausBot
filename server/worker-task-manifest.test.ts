...
function worker(platform: WorkerPlatform, overrides: Partial<ResolvedWorker> = {}): ResolvedWorker {
  ...
  return {
    ...
    expectedParkedCapabilitySha256: "b".repeat(64),
...
function proposal(platform: WorkerPlatform, overrides: Record<string, JsonValue> = {}): JsonValue {
  const full = manifest(platform) as Record<string, JsonValue>;
  return {
...
describe.each(["windows", "macos"] as const)("%s task manifest", (platform) => {
  ...
  it("binds a model proposal to the selected worker, conversation and current time", () => {
    const w = worker(platform);
    const parsed = parseWorkerTaskProposal(proposal(platform), w, "bound-thread", NOW);
    expect(parsed).toMatchObject({
      ...
      target: {
        sshAlias: w.sshAlias,
        basePolicySha256: w.expectedBasePolicySha256,
      },
    });
  });

  it("refuses a proposal that tries to choose control-plane identity or routing", () => {
    const w = worker(platform);
    expect(() => parseWorkerTaskProposal(
      proposal(platform, { threadId: "other", target: { sshAlias: "other" } }),
      w,
      "bound-thread",
      NOW,
    )).toThrow(/Invalid worker task proposal/);
  });
...
describe("WorkerTaskRegistry", () => {
  ...
  it("drops approval when the document changes under the same task id", () => {
    ...
    const changed = parse("macos", {
      commands: [{ id: "build", executable: "/opt/homebrew/bin/just", argv: ["test"], cwd: "src", timeoutMs: 60_000 }],
    });
  ...
  it("refuses a task id collision across conversations or workers", () => {
    const registry = new WorkerTaskRegistry();
    registry.register(record());
    expect(() => registry.register(parse("macos", { threadId: "thread-2" })))
      .toThrow(/already bound/);
    expect(() => registry.register(parseWorkerTaskManifest(
      manifest("windows"),
      worker("windows"),
      NOW,
    ))).toThrow(/already bound/);
