...
export const WORKER_TASK_DEFAULT_LIFETIME_MS = 60 * 60_000;
...
/** The model supplies only task intent. Worker identity, SSH routing,
 * conversation identity and absolute timestamps belong to the trusted control
 * plane and are deliberately absent from this schema. */
const proposalSchema = z.object({
  version: z.literal(WORKER_TASK_MANIFEST_VERSION),
  surface: z.enum(["browser", "desktop"]).optional(),
  taskId: z.string().regex(ID),
  lifetimeMs: z.number().int().min(60_000).max(WORKER_TASK_MAX_LIFETIME_MS).optional(),
  files: z.array(fileSchema).max(512),
  commands: z.array(commandSchema).min(1).max(128),
  origins: z.array(z.string().max(2048)).max(128),
  resultPaths: z.array(relativePath).min(2).max(256),
...
/** Bind an untrusted task proposal to the worker and conversation selected by
 * OpenMausBot. This is the only proposal path exposed to a bot, so neither an
 * SSH alias nor a policy digest has to cross the MCP boundary. */
export function parseWorkerTaskProposal(
  value: JsonValue,
  worker: ResolvedWorker,
  threadId: string,
  now = Date.now(),
  ...
  if (!parsed.success) throw new Error(`Invalid worker task proposal: ${z.prettifyError(parsed.error)}`);
  if (!worker.configured || !worker.expectedBasePolicySha256) {
    throw new Error("Worker task target has no pinned base policy");
  }
  const { lifetimeMs = WORKER_TASK_DEFAULT_LIFETIME_MS, ...proposal } = parsed.data;
  return parseWorkerTaskManifest({
    ...proposal,
    ...
    target: {
      sshAlias: worker.sshAlias,
      basePolicySha256: worker.expectedBasePolicySha256,
    },
  }, worker, now);
}
...
export function verifyWorkerTaskFiles(root: string, manifest: WorkerTaskManifest): void {
  for (const file of manifest.files) readVerifiedWorkerTaskFile(root, file);
}

/** Open and copy one approved file through the same descriptor that is
 * checked and hashed. The returned bytes can be streamed later without a
 * path re-open, closing the verify-then-symlink-swap gap. */
export function readVerifiedWorkerTaskFile(root: string, file: WorkerTaskFile): Buffer {
  ...
  try {
    ...
    if (
      ...
      throw new Error(`Worker task file is not a stable regular file: ${file.path}`);
    }
    ...
    if (content.length !== file.size) throw new Error(`Worker task file size changed: ${file.path}`);
...
export class WorkerTaskRegistry {
  ...
  register(manifest: WorkerTaskManifest): WorkerTaskRecord {
    ...
    if (
      current &&
      (current.manifest.threadId !== manifest.threadId || current.manifest.workerId !== manifest.workerId)
    ) {
      throw new Error("Worker task id is already bound to another conversation or worker");
    }
