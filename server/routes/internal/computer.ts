...
export type ComputerCtx = InternalRequestCtx & {
  ...
  LAZY_VM_CLAIM_GRACE_MS: InternalRoutesOptions["LAZY_VM_CLAIM_GRACE_MS"];
...
export async function computerControlRoute(ctx: ComputerCtx, res: ServerResponse, url: URL, method: string): Promise<boolean> {
  const {
    ...
    activeGroupTurnForBot, notify, claimTurnResource, LAZY_VM_CLAIM_GRACE_MS, internalCapability, json, readInternalBody,
  } = ctx;
