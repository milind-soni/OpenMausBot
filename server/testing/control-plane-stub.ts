// A stand-in for cloudflare/control-plane for tests of `astra login`
...
export async function startControlPlaneStub(options: { otp?: string; endpointUrl?: string } = {}): Promise<ControlPlaneStub> {
  ...
  const endpointUrl = options.endpointUrl ?? "https://c-stub.astra.invalid";
  ...
  const server = createServer(async (req, res) => {
    ...
    if (method === "GET" && path === "/healthz") return send(200, { ok: true, service: "astra-control-plane" });
