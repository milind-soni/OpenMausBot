...
Open **Connected apps** from the sidebar, choose a service, and complete its authorization in your normal browser. Astra turns the resulting connection into tools that supported agent engines can use.

<ProductScreenshot name="docs-connected-apps" alt="The clean Astra connected-app marketplace" caption="Search the marketplace or review every account from the Connected tab." />

## Official packaged app

The signed desktop app registers its own installation with Astra's managed connected-apps service. You do **not** need to find or paste a Composio key.
...
The official managed Astra service keeps X disabled until it owns an appropriate X OAuth app. Older Astra tasks that refer to the connector as `x` continue to work; Astra translates that alias to Composio's `twitter` toolkit automatically.

## Multiple accounts

Connect more than one account for the same toolkit and give each a clear alias such as `work`, `personal`, or `client-acme`. When several accounts could perform an action, Astra requires explicit selection instead of silently replacing the first connection.
