...
```bash
pnpm docs:build
pnpm --filter @astra/docs types:check
pnpm --filter @astra/docs lint
```

The changelog reads published releases from `milind-soni/Astra` plus the
...
Create a second Vercel project beside the existing `astra.com` project:

1. Import the `milind-soni/Astra` repository.
...
5. Add `docs.astra.com` under **Settings → Domains**.
...
preview URLs for documentation pull requests. Keep `astra.com` on the
