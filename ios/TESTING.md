...
```sh
git clone https://github.com/milind-soni/Astra
cd Astra
...
dns-sd -B _astra._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/Astra must be allowed, or the phone reaches
...
```sh
xcodebuild -project AstraCompanion.xcodeproj -scheme AstraCompanion \
...
1. **Pair.** In Astra → Settings → Phone, choose **Pair a phone**.
   Scan the QR code with the phone's Camera, open AstraMobile,
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → Astra → Local Network. If the
...
3. **In Astra → Settings → Phone:** with Phone access on, the panel now
...
- **Xcode console**, subsystem `com.astra.companion`: `opening stream`,
