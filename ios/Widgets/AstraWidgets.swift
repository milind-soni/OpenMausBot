...
@main
struct AstraWidgets: WidgetBundle {
    ...
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: BotActivityAttributes.self) { context in
            ...
            DynamicIsland {
                ...
                AstraFaceStill(color: context.attributes.color, state: AstraState(rawValue: context.state.face) ?? .idle, size: 24)
            } compactTrailing: {
                compactTrailing(context)
            } minimal: {
                AstraFaceStill(color: context.attributes.color, state: AstraState(rawValue: context.state.face) ?? .idle, size: 22)
            }
            .keylineTint(AstraPalette.color(context.attributes.color))
        }
    }
    ...
    private func compactTrailing(_ context: ActivityViewContext<BotActivityAttributes>) -> some View {
        ...
        case "needsYou":
            Image(systemName: "hand.raised.fill")
                ...
                .foregroundStyle(AstraPalette.color(context.attributes.color))
        case "working":
            Image(systemName: "circle.dotted")
                .font(.system(size: 13, weight: .semibold))
                .foregroundStyle(.white.opacity(0.8))
        default:
            Circle().fill(AstraPalette.color(context.attributes.color)).frame(width: 8, height: 8)
        }
    }
}
                        ...
                        .background(
                            Capsule().fill(
                                option.caseInsensitiveCompare("Deny") == .orderedSame
                                    ...
                                    : AstraPalette.color(context.attributes.color)
                            )
                        )
                }
                .buttonStyle(.plain)
...
private struct OrbitingFace: View {
    ...
    var body: some View {
        ZStack {
            ...
            AstraFaceStill(color: context.attributes.color, state: AstraState(rawValue: context.state.face) ?? .idle, size: size, comets: true, at: Date())
        }
    }
}
