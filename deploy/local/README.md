...
`ASTRA_HTTP_PORT` changes the host port and the default public URL.
...
For Tailscale Serve, set `ASTRA_PUBLIC_URL` to your HTTPS URL and
`ASTRA_HTTPS_HOST` to its hostname without scheme or path. Configure Tailscale
Serve on the host to forward to the chosen localhost HTTP port.
Keep the default `ASTRA_BIND_ADDRESS=127.0.0.1`. Caddy refuses to start when
`ASTRA_HTTPS_HOST` is set with any other bind address, so direct remote HTTP
...
Without `ASTRA_HTTPS_HOST`, the bind address can be changed for HTTP access.
...
docker compose exec omb node dist-server/astra.js pair
```

On Windows, `./astra.ps1` forwards arguments to Compose using the repository
...
volume, set `ASTRA_DATA_VOLUME` to its name and `ASTRA_DATA_EXTERNAL=true`.
