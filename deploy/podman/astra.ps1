...
if (-not $args.Count -or $args[0] -in @('--help', '-h', 'help')) {
    Write-Output 'Usage: .\deploy\podman\astra.ps1 setup | <compose arguments>'
    Write-Output 'Example: .\deploy\podman\astra.ps1 up -d --build'
    Write-Output 'Options: ASTRA_PODMAN_MACHINE (default astra), ASTRA_PODMAN_ENV_FILE (default .env)'
    exit 0
}
...
$machine = if ($env:ASTRA_PODMAN_MACHINE) { $env:ASTRA_PODMAN_MACHINE } else { 'astra' }
...
$envFile = if ($env:ASTRA_PODMAN_ENV_FILE) { $env:ASTRA_PODMAN_ENV_FILE } else { '.env' }
